...
class Solution:
    ...
    # O(n) without extra space to track path and compare lowest node
    def lowestCommonAncestor(self, root: TreeNode, p: TreeNode, q: TreeNode) -> TreeNode:
        
        def find_path(currNode: TreeNode, nodeToFind: TreeNode, path: list[TreeNode]):
            ...
            leftFound = find_path(currNode.left, nodeToFind, path)

            rightFound = find_path(currNode.right, nodeToFind, path)

            # either left or right child found node, include this currNode in path
        ...
        pathP = []
        find_path(root, p, pathP)

        pathQ = []
        find_path(root, q, pathQ)

        i = 0
        while i <= min(len(pathP) - 1, len(pathQ) - 1):
            if pathP[i] == pathQ[i]:
                i += 1
            else:
                break
...
r = s.lowestCommonAncestor(n3, n5, n4)
print(r.val)
...
n1.left = n2
n1.right = n3
s.lowestCommonAncestor(n1, n3, n2)
